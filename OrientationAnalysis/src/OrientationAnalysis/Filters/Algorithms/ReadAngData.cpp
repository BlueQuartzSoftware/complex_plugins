...
void ReadAngData::copyRawEbsdData(AngReader* reader) const
{
  ...
  ImageGeom& imageGeom = m_DataStructure.getDataRefAs<ImageGeom>(m_InputValues->dataContainerPath);
  ...
  std::vector<size_t> tDims = {
      imageGeom.getNumXPoints(),
      imageGeom.getNumYPoints(),
      imageGeom.getNumZPoints()
  };
  ...
  // Adjust the values of the 'phase' data to correct for invalid values
  {
    ...
    Int32Array& targetArray = m_DataStructure.getDataRefAs<Int32Array>(m_InputValues->cellAttributeMatrixPath.createChildPath(EbsdLib::AngFile::Phases));
    targetArray.fill({phasePtr, totalPoints});
  }


  float* fComp0 = nullptr;
  float* fComp1 = nullptr;
  float* fComp2 = nullptr;
  // Condense the Euler Angles from 3 separate arrays into a single 1x3 array
  {
    fComp0 = reinterpret_cast<float*>(reader->getPointerByName(EbsdLib::Ang::Phi1));
    fComp1 = reinterpret_cast<float*>(reader->getPointerByName(EbsdLib::Ang::Phi));
    fComp2 = reinterpret_cast<float*>(reader->getPointerByName(EbsdLib::Ang::Phi2));
    cDims[0] = 3;
  ...
  cDims[0] = 1;
  {
    fComp0 = reinterpret_cast<float*>(reader->getPointerByName(EbsdLib::Ang::ImageQuality));
    ...
    targetArray.fill({fComp0, totalPoints});
  }

  {
    fComp0 = reinterpret_cast<float*>(reader->getPointerByName(EbsdLib::Ang::ConfidenceIndex));
  ...
  {
    fComp0 = reinterpret_cast<float*>(reader->getPointerByName(EbsdLib::Ang::SEMSignal));
  ...
  {
    fComp0 = reinterpret_cast<float*>(reader->getPointerByName(EbsdLib::Ang::Fit));
  ...
  {
    fComp0 = reinterpret_cast<float*>(reader->getPointerByName(EbsdLib::Ang::XPosition));
  ...
  {
    fComp0 = reinterpret_cast<float*>(reader->getPointerByName(EbsdLib::Ang::YPosition));
