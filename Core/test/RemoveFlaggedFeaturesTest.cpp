...
{ // Instantiate the filter, a DataStructure object and an Arguments Object
  ...
  ImageGeom* imageGeom = ImageGeom::Create(dataGraph, Constants::k_DataContainer);
  ...
  auto* attributeMatrix = AttributeMatrix::Create(dataGraph, Constants::k_CellData, imageGeom->getId());
  std::vector<size_t> tupleDims(dims.rbegin(), dims.rend());
  attributeMatrix->setShape(tupleDims);
  imageGeom->setCellData(*attributeMatrix);

  Int32Array* featureIds = UnitTest::CreateTestDataArray<int32>(dataGraph, Constants::k_FeatureIds, tupleDims, {1}, attributeMatrix->getId());
  ...
  auto* featureAttributeMatrix = AttributeMatrix::Create(dataGraph, Constants::k_CellFeatureData, imageGeom->getId());
  featureAttributeMatrix->setShape({4});
  BoolArray* maskArray = BoolArray::CreateWithStore<DataStore<bool>>(dataGraph, Constants::k_ActiveName, {4}, {1}, featureAttributeMatrix->getId());
  ...
  Int32Array* testArray = UnitTest::CreateTestDataArray<int32>(dataGraph, Constants::k_Int32DataSet, {4}, {1}, featureAttributeMatrix->getId());
  ...
  Int32Array& featureIdsResult = dataGraph.getDataRefAs<Int32Array>(featureIdsPath);
  ...
  AttributeMatrix& cellFeatureAMResult = dataGraph.getDataRefAs<AttributeMatrix>(DataPath({Constants::k_DataContainer, Constants::k_CellFeatureData}));
  REQUIRE(cellFeatureAMResult.getNumTuples() == 3);

  Int32Array& testArrayResult = dataGraph.getDataRefAs<Int32Array>(DataPath({Constants::k_DataContainer, Constants::k_CellFeatureData, Constants::k_Int32DataSet}));
