...
private:
    ...
    void Test(uint64 fileType, uint64 delimiter)
    {
        ...
        {
          UnitTest::CreateTestDataArray<T>(m_DataStructure, std::to_string(fileType) + "_" + std::to_string(delimiter) + "_" + "array_" + std::to_string(index), {k_NumOfTuples}, {k_NumComponents})->fill(m_FillValue + static_cast<T>(2.0 * index));
        }
