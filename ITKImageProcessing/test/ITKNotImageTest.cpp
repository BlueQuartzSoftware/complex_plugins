...
TEST_CASE("ITK::NotImageFilter(defaults)", "[ITKImageProcessing][ITKNotImage][defaults]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKNotImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_NotImageFilter_defaults.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_NotImageFilter_defaults");
  } // End Scope Section
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "NotImageFilter_defaults.h5";
