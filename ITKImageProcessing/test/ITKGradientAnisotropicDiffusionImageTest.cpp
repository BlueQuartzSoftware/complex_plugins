...
TEST_CASE("ITK::GradientAnisotropicDiffusionImageFilter(defaults)","[ITKImageProcessing][ITKGradientAnisotropicDiffusionImage][defaults]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKGradientAnisotropicDiffusionImage::k_TimeStep_Key, std::make_any<double>(0.01));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_GradientAnisotropicDiffusionImageFilter_defaults");
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_GradientAnisotropicDiffusionImageFilter_defaults.nrrd";
...
TEST_CASE("ITK::GradientAnisotropicDiffusionImageFilter(longer)","[ITKImageProcessing][ITKGradientAnisotropicDiffusionImage][longer]")
{
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    // Remove *all* files generated by this test
    fs::path testDir = fs::path(unit_test::k_BinaryDir.view()) / "test";
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_GradientAnisotropicDiffusionImageFilter_longer");
    REQUIRE(error == 0);
