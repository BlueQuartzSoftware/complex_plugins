...
TEST_CASE("ITK::StandardDeviationProjectionImageFilter(z_projection)","[ITKImageProcessing][ITKStandardDeviationProjectionImage][z_projection]")
{
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_StandardDeviationProjectionImageFilter_z_projection.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_StandardDeviationProjectionImageFilter_z_projection");
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_StandardDeviationProjectionImageFilter_z_projection.nrrd";
...
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "StandardDeviationProjectionImageFilter_z_projection.h5";
