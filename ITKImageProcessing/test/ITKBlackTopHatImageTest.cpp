...
// Test black top hat
TEST_CASE("ITK::BlackTopHatImageFilter(BlackTopHapErode)", "[ITKImageProcessing][ITKBlackTopHatImage][BlackTopHapErode]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKBlackTopHatImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    auto pKernelRadius = std::vector<uint32_t>(3, 1);
    args.insertOrAssign(ITKBlackTopHatImage::k_KernelRadius_Key, std::make_any<VectorUInt32Parameter::ValueType>(pKernelRadius));
    itk::simple::KernelEnum pKernelType = itk::simple::sitkBall;
    args.insertOrAssign(ITKBlackTopHatImage::k_KernelType_Key, std::make_any<ChoicesParameter::ValueType>(pKernelType));
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_BlackTopHatImageFilter_BlackTopHapErode.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_BlackTopHatImageFilter_BlackTopHapErode");
  } // End Scope Section
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "BlackTopHatImageFilter_BlackTopHapErode.h5";
