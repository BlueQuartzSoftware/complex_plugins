...
TEST_CASE("ITK::MinMaxCurvatureFlowImageFilter(defaults)","[ITKImageProcessing][ITKMinMaxCurvatureFlowImage][defaults]")
{
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_MinMaxCurvatureFlowImageFilter_defaults");
    REQUIRE(error == 0);
...
TEST_CASE("ITK::MinMaxCurvatureFlowImageFilter(longer)","[ITKImageProcessing][ITKMinMaxCurvatureFlowImage][longer]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKMinMaxCurvatureFlowImage::k_SelectedImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKMinMaxCurvatureFlowImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    args.insertOrAssign(ITKMinMaxCurvatureFlowImage::k_TimeStep_Key, std::make_any<double>(0.1));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    // Remove *all* files generated by this test
    fs::path testDir = fs::path(unit_test::k_BinaryDir.view()) / "test";
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_MinMaxCurvatureFlowImageFilter_longer");
    REQUIRE(error == 0);
