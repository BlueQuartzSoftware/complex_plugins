...
TEST_CASE("ITKBilateralImageFilter(3d)", "[ITKImageProcessing][ITKBilateralImage][3d]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKBilateralImage::k_SelectedImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKBilateralImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    args.insertOrAssign(ITKBilateralImage::k_DomainSigma_Key, std::make_any<float64>(2.0));
    args.insertOrAssign(ITKBilateralImage::k_RangeSigma_Key, std::make_any<float64>(500));
    // Preflight the filter and check result
    auto preflightResult = filter.preflight(ds, args);
    COMPLEX_RESULT_REQUIRE_VALID(preflightResult.outputActions);
    // Execute the filter and check the result
