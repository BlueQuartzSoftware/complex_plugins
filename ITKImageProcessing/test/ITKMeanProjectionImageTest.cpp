...
TEST_CASE("ITKMeanProjectionImageFilter(z_projection)", "[ITKImageProcessing][ITKMeanProjectionImage][z_projection]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKMeanProjectionImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    args.insertOrAssign(ITKMeanProjectionImage::k_ProjectionDimension_Key, std::make_any<unsigned int>(2));
  ...
  {
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_MeanProjectionImageFilter_z_projection.nrrd";
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_MeanProjectionImageFilter_z_projection.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_MeanProjectionImageFilter_z_projection");
  } // End Scope Section
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "MeanProjectionImageFilter_z_projection.h5";
