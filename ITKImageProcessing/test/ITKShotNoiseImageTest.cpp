...
TEST_CASE("ITK::ShotNoiseImageFilter(2d)","[ITKImageProcessing][ITKShotNoiseImage][2d]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKShotNoiseImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    args.insertOrAssign(ITKShotNoiseImage::k_Seed_Key, std::make_any<uint32_t>(123));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_ShotNoiseImageFilter_2d");
    REQUIRE(error == 0);
...
TEST_CASE("ITK::ShotNoiseImageFilter(3d)","[ITKImageProcessing][ITKShotNoiseImage][3d]")
{
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    // Remove *all* files generated by this test
    fs::path testDir = fs::path(unit_test::k_BinaryDir.view()) / "test";
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_ShotNoiseImageFilter_3d");
    REQUIRE(error == 0);
...
TEST_CASE("ITK::ShotNoiseImageFilter(rgb)","[ITKImageProcessing][ITKShotNoiseImage][rgb]")
{
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    // Remove *all* files generated by this test
    fs::path testDir = fs::path(unit_test::k_BinaryDir.view()) / "test";
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_ShotNoiseImageFilter_rgb");
    REQUIRE(error == 0);
