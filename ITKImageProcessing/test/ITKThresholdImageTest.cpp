...
// Default
TEST_CASE("ITK::ThresholdImageFilter(Default)", "[ITKImageProcessing][ITKThresholdImage][Default]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKThresholdImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  // Write the output data to a file
  {
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_ThresholdImageFilter_Default");
  } // End Scope Section
#if 0
  {
...
TEST_CASE("ITK::ThresholdImageFilter(Threshold1)", "[ITKImageProcessing][ITKThresholdImage][Threshold1]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKThresholdImage filter;
  DataStructure ds;
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKThresholdImage::k_SelectedImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKThresholdImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    args.insertOrAssign(ITKThresholdImage::k_Lower_Key, std::make_any<float64>(25000));
    args.insertOrAssign(ITKThresholdImage::k_Upper_Key, std::make_any<float64>(65535));
    // Preflight the filter and check result
    auto preflightResult = filter.preflight(ds, args);
    COMPLEX_RESULT_REQUIRE_VALID(preflightResult.outputActions);
    // Execute the filter and check the result
  ...
  // Write the output data to a file
  {
    ...
    // Remove *all* files generated by this test
    fs::path testDir = fs::path(unit_test::k_BinaryDir.view()) / "test";
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_ThresholdImageFilter_Threshold1");
  } // End Scope Section
#if 0
  {
...
TEST_CASE("ITK::ThresholdImageFilter(Threshold2)", "[ITKImageProcessing][ITKThresholdImage][Threshold2]")
{
  // Instantiate the filter, a DataStructure object and an Arguments Object
  ITKThresholdImage filter;
  DataStructure ds;
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKThresholdImage::k_SelectedImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKThresholdImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    args.insertOrAssign(ITKThresholdImage::k_Lower_Key, std::make_any<float64>(0));
    args.insertOrAssign(ITKThresholdImage::k_Upper_Key, std::make_any<float64>(25000));
    args.insertOrAssign(ITKThresholdImage::k_OutsideValue_Key, std::make_any<float64>(25000));
    // Preflight the filter and check result
    auto preflightResult = filter.preflight(ds, args);
    COMPLEX_RESULT_REQUIRE_VALID(preflightResult.outputActions);
    // Execute the filter and check the result
  ...
  // Write the output data to a file
  {
    ...
    // Remove *all* files generated by this test
    fs::path testDir = fs::path(unit_test::k_BinaryDir.view()) / "test";
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_ThresholdImageFilter_Threshold2");
  } // End Scope Section
#if 0
  {
