...
TEST_CASE("ITK::GrayscaleMorphologicalClosingImageFilter(GrayscaleMorphologicalClosing)", "[ITKImageProcessing][ITKGrayscaleMorphologicalClosingImage][GrayscaleMorphologicalClosing]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKGrayscaleMorphologicalClosingImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    auto pKernelRadius = std::vector<uint32_t>(3, 1);
    args.insertOrAssign(ITKGrayscaleMorphologicalClosingImage::k_KernelRadius_Key, std::make_any<VectorUInt32Parameter::ValueType>(pKernelRadius));
  ...
  // Write the output data to a file
  {
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_GrayscaleMorphologicalClosingImageFilter_GrayscaleMorphologicalClosing");
  } // End Scope Section
#if 0
  {
