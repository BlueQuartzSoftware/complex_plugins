...
TEST_CASE("ITK::Log10ImageFilter(defaults)","[ITKImageProcessing][ITKLog10Image][defaults]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKLog10Image::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_Log10ImageFilter_defaults.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_Log10ImageFilter_defaults");
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_Log10ImageFilter_defaults.nrrd";
...
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "Log10ImageFilter_defaults.h5";
