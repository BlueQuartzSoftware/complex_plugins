...
TEST_CASE("ITKLabelContourImageFilter(default)", "[ITKImageProcessing][ITKLabelContourImage][default]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKLabelContourImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_LabelContourImageFilter_default.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_LabelContourImageFilter_default");
  } // End Scope Section
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "LabelContourImageFilter_default.h5";
