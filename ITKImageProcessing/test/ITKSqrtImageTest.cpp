...
TEST_CASE("ITKImageProcessing::ITKSqrtImageFilter(defaults)", "[ITKImageProcessing][ITKSqrtImage][defaults]")
{
  DataStructure ds;
  ITKSqrtImage filter;
  ...
  Arguments args;
  args.insertOrAssign(ITKSqrtImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
  args.insertOrAssign(ITKSqrtImage::k_SelectedImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
  args.insertOrAssign(ITKSqrtImage::k_OutputImageDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.view() / "JSONFilters/Baseline/BasicFilters_SqrtImageFilter_defaults.nrrd";
