...
TEST_CASE("ITK::DanielssonDistanceMapImageFilter(default)", "[ITKImageProcessing][ITKDanielssonDistanceMapImage][default]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKDanielssonDistanceMapImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  {
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_DanielssonDistanceMapImageFilter_default.nrrd";
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_DanielssonDistanceMapImageFilter_default.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_DanielssonDistanceMapImageFilter_default");
  } // End Scope Section
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "DanielssonDistanceMapImageFilter_default.h5";
