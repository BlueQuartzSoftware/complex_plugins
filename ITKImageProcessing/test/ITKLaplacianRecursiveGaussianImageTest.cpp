...
TEST_CASE("ITK::LaplacianRecursiveGaussianImageFilter(default)","[ITKImageProcessing][ITKLaplacianRecursiveGaussianImage][default]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKLaplacianRecursiveGaussianImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_LaplacianRecursiveGaussianImageFilter_default.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_LaplacianRecursiveGaussianImageFilter_default");
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_LaplacianRecursiveGaussianImageFilter_default.nrrd";
...
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "LaplacianRecursiveGaussianImageFilter_default.h5";
