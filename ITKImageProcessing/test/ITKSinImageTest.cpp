...
TEST_CASE("ITKImageProcessing::ITKSinImageFilter(defaults)", "[ITKImageProcessing][ITKSinImage][defaults]")
{
  ...
  Arguments args;
  args.insertOrAssign(ITKSinImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
  args.insertOrAssign(ITKSinImage::k_SelectedImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
  args.insertOrAssign(ITKSinImage::k_OutputImageDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.view() / "JSONFilters/Baseline/BasicFilters_SinImageFilter_defaults.nrrd";
