...
TEST_CASE("ITK::BinaryDilateImageFilter(BinaryDilate)", "[ITKImageProcessing][ITKBinaryDilateImage][BinaryDilate]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKBinaryDilateImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    auto pKernelRadius = std::vector<uint32_t>(3, 1);
    args.insertOrAssign(ITKBinaryDilateImage::k_KernelRadius_Key, std::make_any<VectorUInt32Parameter::ValueType>(pKernelRadius));
    ...
    args.insertOrAssign(ITKBinaryDilateImage::k_ForegroundValue_Key, std::make_any<float64>(255.0));
  ...
  // Write the output data to a file
  {
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_BinaryDilateImageFilter_BinaryDilate");
  } // End Scope Section
#if 0
  {
...
TEST_CASE("ITK::BinaryDilateImageFilter(BinaryDilateVectorRadius)", "[ITKImageProcessing][ITKBinaryDilateImage][BinaryDilateVectorRadius]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKBinaryDilateImage::k_SelectedImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
    args.insertOrAssign(ITKBinaryDilateImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    auto pKernelRadius = std::vector<uint32_t>{20, 1};
    args.insertOrAssign(ITKBinaryDilateImage::k_KernelRadius_Key, std::make_any<VectorUInt32Parameter::ValueType>(pKernelRadius));
    itk::simple::KernelEnum pKernelType = itk::simple::sitkBox;
    args.insertOrAssign(ITKBinaryDilateImage::k_KernelType_Key, std::make_any<ChoicesParameter::ValueType>(pKernelType));
    args.insertOrAssign(ITKBinaryDilateImage::k_ForegroundValue_Key, std::make_any<float64>(255));
    // Preflight the filter and check result
    auto preflightResult = filter.preflight(ds, args);
    COMPLEX_RESULT_REQUIRE_VALID(preflightResult.outputActions);
    // Execute the filter and check the result
  ...
  // Write the output data to a file
  {
    ...
    // Remove *all* files generated by this test
    fs::path testDir = fs::path(unit_test::k_BinaryDir.view()) / "test";
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_BinaryDilateImageFilter_BinaryDilateVectorRadius");
  } // End Scope Section
#if 0
  {
