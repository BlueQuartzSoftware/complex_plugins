...
TEST_CASE("ITKImageProcessing::ITKLogImageFilter(defaults)", "[ITKImageProcessing][ITKLogImage][defaults]")
{
  DataStructure ds;
  ITKLogImage filter;
  ...
  Arguments args;
  args.insertOrAssign(ITKLogImage::k_SelectedImageGeomPath_Key, std::make_any<DataPath>(inputGeometryPath));
  args.insertOrAssign(ITKLogImage::k_SelectedImageDataPath_Key, std::make_any<DataPath>(inputDataPath));
  args.insertOrAssign(ITKLogImage::k_OutputImageDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.view() / "JSONFilters/Baseline/BasicFilters_LogImageFilter_defaults.nrrd";
