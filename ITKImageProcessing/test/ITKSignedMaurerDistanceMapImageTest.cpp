...
TEST_CASE("ITK::SignedMaurerDistanceMapImageFilter(default)","[ITKImageProcessing][ITKSignedMaurerDistanceMapImage][default]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKSignedMaurerDistanceMapImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_SignedMaurerDistanceMapImageFilter_default.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_SignedMaurerDistanceMapImageFilter_default");
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_SignedMaurerDistanceMapImageFilter_default.nrrd";
...
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "SignedMaurerDistanceMapImageFilter_default.h5";
