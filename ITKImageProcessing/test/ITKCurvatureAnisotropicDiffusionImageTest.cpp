...
TEST_CASE("ITK::CurvatureAnisotropicDiffusionImageFilter(defaults)","[ITKImageProcessing][ITKCurvatureAnisotropicDiffusionImage][defaults]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKCurvatureAnisotropicDiffusionImage::k_TimeStep_Key, std::make_any<double>(0.01));
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_CurvatureAnisotropicDiffusionImageFilter_defaults.nrrd";
...
TEST_CASE("ITK::CurvatureAnisotropicDiffusionImageFilter(longer)","[ITKImageProcessing][ITKCurvatureAnisotropicDiffusionImage][longer]")
{
  ...
  // Write the output data to a file, read and compare to baseline image
  {
    ...
    REQUIRE(error == 0);
    fs::path baselineFilePath = fs::path(unit_test::k_SourceDir.view()) / complex::unit_test::k_DataDir.str() / "JSONFilters/Baseline/BasicFilters_CurvatureAnisotropicDiffusionImageFilter_longer.nrrd";
    DataPath baselineGeometryPath({complex::ITKTestBase::k_BaselineGeometryPath});
    DataPath baselineDataPath = baselineGeometryPath.createChildPath(complex::ITKTestBase::k_BaselineDataPath);
    error = complex::ITKTestBase::ReadImage(ds, baselineFilePath, baselineGeometryPath, baselineDataPath);
    REQUIRE(error == 0);
