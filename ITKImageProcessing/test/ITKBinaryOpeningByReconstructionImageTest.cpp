...
TEST_CASE("ITK::BinaryOpeningByReconstructionImageFilter(BinaryOpeningByReconstruction)", "[ITKImageProcessing][ITKBinaryOpeningByReconstructionImage][BinaryOpeningByReconstruction]")
{
  ...
  // Test the filter itself
  {
    ...
    args.insertOrAssign(ITKBinaryOpeningByReconstructionImage::k_OutputIamgeDataPath_Key, std::make_any<DataPath>(outputDataPath));
    auto pKernelRadius = std::vector<uint32_t>(3, 5);
    args.insertOrAssign(ITKBinaryOpeningByReconstructionImage::k_KernelRadius_Key, std::make_any<VectorUInt32Parameter::ValueType>(pKernelRadius));
    itk::simple::KernelEnum pKernelType = itk::simple::sitkBall;
    args.insertOrAssign(ITKBinaryOpeningByReconstructionImage::k_KernelType_Key, std::make_any<ChoicesParameter::ValueType>(pKernelType));
    args.insertOrAssign(ITKBinaryOpeningByReconstructionImage::k_ForegroundValue_Key, std::make_any<float64>(200));
  ...
  {
    fs::path filePath = fs::path(unit_test::k_BinaryDir.view()) / "test/BasicFilters_BinaryOpeningByReconstructionImageFilter_BinaryOpeningByReconstruction.nrrd";
    DataPath inputGeometryPath({complex::ITKTestBase::k_ImageGeometryPath});
    DataPath outputDataPath = inputGeometryPath.createChildPath(complex::ITKTestBase::k_OutputDataPath);
    ...
    ITKTestBase::RemoveFiles(testDir, "BasicFilters_BinaryOpeningByReconstructionImageFilter_BinaryOpeningByReconstruction");
  } // End Scope Section
#if 0
  {
    fs::path filePath =fs::path( unit_test::k_BinaryDir.view()) / "test" / "BinaryOpeningByReconstructionImageFilter_BinaryOpeningByReconstruction.h5";
