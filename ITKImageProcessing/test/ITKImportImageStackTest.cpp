...
TEST_CASE("ITKImageProcessing::ITKImportImageStack: CompareImage", "[ITKImageProcessing][ITKImportImageStack]")
{
  ...
  const auto* imageGeom = dataGraph.getDataAs<ImageGeom>(k_ImageGeomPath);
  ...
  const auto* imageData = dataGraph.getDataAs<UInt8Array>(k_ImageDataPath);
  REQUIRE(imageData != nullptr);

  std::string md5Hash = ITKTestBase::ComputeMd5Hash(dataGraph, k_ImageDataPath);
