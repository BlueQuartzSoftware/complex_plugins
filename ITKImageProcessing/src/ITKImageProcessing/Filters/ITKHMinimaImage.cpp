...
#define ITK_ARRAY_HELPER_NAMESPACE HMinimaImage
...
namespace
{
struct ITKHMinimaImageCreationFunctor
{
  float64 pHeight = 2.0;
  ...
  auto operator()() const
  {
    using FilterType = itk::HMinimaImageFilter<InputImageType, OutputImageType>;
    typename FilterType::Pointer filter = FilterType::New();
    filter->SetHeight(pHeight);
...
IFilter::PreflightResult ITKHMinimaImage::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler) const
{
  ...
  // store those actions.
  complex::Result<OutputActions> resultOutputActions = HMinimaImage::ITK::DataCheck(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath);
...
Result<> ITKHMinimaImage::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler) const
{
   ...
   ***************************************************************************/
  ::ITKHMinimaImageCreationFunctor itkFunctor = {pHeight, pFullyConnected};
   ...
   ***************************************************************************/
  return HMinimaImage::ITK::Execute(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath, itkFunctor);
}
} // namespace complex
