...
#define ITK_ARRAY_HELPER_NAMESPACE ThresholdImage
...
namespace
{
struct ITKThresholdImageCreationFunctor
{
  float64 pLower = 0.0;
  float64 pUpper = 1.0;
  float64 pOutsideValue = 0.0;
  ...
  auto operator()() const
  {
    ...
    filter->SetLower(pLower);
    filter->SetUpper(pUpper);
...
IFilter::PreflightResult ITKThresholdImage::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler) const
{
  ...
  auto pLower = filterArgs.value<float64>(k_Lower_Key);
  auto pUpper = filterArgs.value<float64>(k_Upper_Key);
  ...
  // store those actions.
  complex::Result<OutputActions> resultOutputActions = ThresholdImage::ITK::DataCheck(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath);
...
Result<> ITKThresholdImage::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler) const
{
   ...
   ***************************************************************************/
  ::ITKThresholdImageCreationFunctor itkFunctor = {pLower, pUpper, pOutsideValue};
   ...
   ***************************************************************************/
  return ThresholdImage::ITK::Execute(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath, itkFunctor);
}
} // namespace complex
