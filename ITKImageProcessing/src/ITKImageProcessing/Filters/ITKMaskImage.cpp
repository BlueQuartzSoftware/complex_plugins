...
#define ITK_ARRAY_HELPER_NAMESPACE MaskImage
...
{
  double pOutsideValue = 0;
  double pMaskingValue = 0;
...
IFilter::PreflightResult ITKMaskImage::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler) const
{
  ...
  auto pMaskingValue = filterArgs.value<float64>(k_MaskingValue_Key);
  ...
  // store those actions.
  complex::Result<OutputActions> resultOutputActions = MaskImage::ITK::DataCheck(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath);
...
Result<> ITKMaskImage::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler) const
{
   ...
   ***************************************************************************/
  ::ITKMaskImageCreationFunctor itkFunctor = {pOutsideValue, pMaskingValue};
   ...
   ***************************************************************************/
  return MaskImage::ITK::Execute(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath, itkFunctor);
}
} // namespace complex
