...
#define ITK_ARRAY_HELPER_NAMESPACE IsoContourDistanceImage
...
struct ITKIsoContourDistanceImageCreationFunctor
{
  float64 pLevelSetValue = 0.0;
  float64 pFarValue = 10;
  ...
  auto operator()() const
  {
    ...
    filter->SetLevelSetValue(pLevelSetValue);
    filter->SetFarValue(pFarValue);
    return filter;
  }
};
} // namespace
...
IFilter::PreflightResult ITKIsoContourDistanceImage::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler) const
{
  ...
  // store those actions.
  complex::Result<OutputActions> resultOutputActions = IsoContourDistanceImage::ITK::DataCheck<FilterOutputType>(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath);
...
Result<> ITKIsoContourDistanceImage::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler) const
{
   ...
   ***************************************************************************/
  ::ITKIsoContourDistanceImageCreationFunctor itkFunctor = {pLevelSetValue, pFarValue};
   ...
   ***************************************************************************/
  return IsoContourDistanceImage::ITK::Execute<ITKIsoContourDistanceImageCreationFunctor, FilterOutputType>(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath, itkFunctor);
}
} // namespace complex
