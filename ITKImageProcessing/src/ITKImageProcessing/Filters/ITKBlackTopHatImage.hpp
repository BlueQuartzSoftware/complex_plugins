...
{
/**
 * @class ITKBlackTopHatImage
 * @brief Black top hat extracts local minima that are smaller than the structuring element.
 *
 * Black top hat extracts local minima that are smaller than the structuring element. It subtracts the background from the input image. The output of the filter transforms the black valleys into white
 * peaks.
 *
 ...
 */
class ITKIMAGEPROCESSING_EXPORT ITKBlackTopHatImage : public IFilter
{
public:
  ITKBlackTopHatImage() = default;
  ~ITKBlackTopHatImage() noexcept override = default;

  ITKBlackTopHatImage(const ITKBlackTopHatImage&) = delete;
  ITKBlackTopHatImage(ITKBlackTopHatImage&&) noexcept = delete;

  ITKBlackTopHatImage& operator=(const ITKBlackTopHatImage&) = delete;
  ITKBlackTopHatImage& operator=(ITKBlackTopHatImage&&) noexcept = delete;
...
COMPLEX_DEF_FILTER_TRAITS(complex, ITKBlackTopHatImage, "e26e7359-f72c-5924-b42e-dd5dd454a794");
