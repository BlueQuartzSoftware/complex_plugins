...
#define ITK_ARRAY_HELPER_NAMESPACE RelabelComponentImage
...
struct ITKRelabelComponentImageCreationFunctor
{
  ...
  auto operator()() const
  {
    ...
    filter->SetMinimumObjectSize(pMinimumObjectSize);
    filter->SetSortByObjectSize(pSortByObjectSize);
    return filter;
  }
};
} // namespace
...
IFilter::PreflightResult ITKRelabelComponentImage::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler) const
{
  ...
  auto pMinimumObjectSize = filterArgs.value<uint64_t>(k_MinimumObjectSize_Key);
  ...
  // store those actions.
  complex::Result<OutputActions> resultOutputActions = RelabelComponentImage::ITK::DataCheck(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath);
...
Result<> ITKRelabelComponentImage::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler) const
{
   ...
   ***************************************************************************/
  return RelabelComponentImage::ITK::Execute(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath, itkFunctor);
}
} // namespace complex
