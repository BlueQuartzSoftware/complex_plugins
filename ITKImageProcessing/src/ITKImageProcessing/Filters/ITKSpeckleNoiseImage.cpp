...
#define ITK_ARRAY_HELPER_NAMESPACE SpeckleNoiseImage
...
namespace
{
struct ITKSpeckleNoiseImageCreationFunctor
{
  float64 pStandardDeviation = 1.0;
  uint32_t pSeed = (uint32_t)itk::simple::sitkWallClock;
  ...
  auto operator()() const
  {
    ...
    filter->SetStandardDeviation(pStandardDeviation);
...
IFilter::PreflightResult ITKSpeckleNoiseImage::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler) const
{
  ...
  auto pStandardDeviation = filterArgs.value<float64>(k_StandardDeviation_Key);
  ...
  // store those actions.
  complex::Result<OutputActions> resultOutputActions = SpeckleNoiseImage::ITK::DataCheck(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath);
...
Result<> ITKSpeckleNoiseImage::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler) const
{
   ...
   ***************************************************************************/
  ::ITKSpeckleNoiseImageCreationFunctor itkFunctor = {pStandardDeviation, pSeed};
   ...
   ***************************************************************************/
  return SpeckleNoiseImage::ITK::Execute(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath, itkFunctor);
}
} // namespace complex
