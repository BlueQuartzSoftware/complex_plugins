...
#define ITK_ARRAY_HELPER_NAMESPACE SigmoidImage
...
namespace
{
struct ITKSigmoidImageCreationFunctor
{
  double pAlpha = 1;
  double pBeta = 0;
...
IFilter::PreflightResult ITKSigmoidImage::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler) const
{
  ...
  // store those actions.
  complex::Result<OutputActions> resultOutputActions = SigmoidImage::ITK::DataCheck(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath);
...
Result<> ITKSigmoidImage::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler) const
{
   ...
   ***************************************************************************/
  ::ITKSigmoidImageCreationFunctor itkFunctor = {pAlpha, pBeta, pOutputMaximum, pOutputMinimum};
   ...
   ***************************************************************************/
  return SigmoidImage::ITK::Execute(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath, itkFunctor);
}
} // namespace complex
