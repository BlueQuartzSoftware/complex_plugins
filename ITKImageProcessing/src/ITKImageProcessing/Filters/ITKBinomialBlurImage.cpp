...
#define ITK_ARRAY_HELPER_NAMESPACE BinomialBlurImage
...
struct ITKBinomialBlurImageCreationFunctor
{
  ...
  auto operator()() const
  {
    ...
    filter->SetRepetitions(pRepetitions);
    return filter;
  }
};
} // namespace
...
IFilter::PreflightResult ITKBinomialBlurImage::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler) const
{
  ...
  auto pRepetitions = filterArgs.value<unsigned int>(k_Repetitions_Key);
  ...
  // store those actions.
  complex::Result<OutputActions> resultOutputActions = BinomialBlurImage::ITK::DataCheck(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath);
...
Result<> ITKBinomialBlurImage::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler) const
{
   ...
   ***************************************************************************/
  return BinomialBlurImage::ITK::Execute(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath, itkFunctor);
}
} // namespace complex
