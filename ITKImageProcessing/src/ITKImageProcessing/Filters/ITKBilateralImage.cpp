...
#define ITK_ARRAY_HELPER_NAMESPACE BilateralImage
...
namespace
{
struct ITKBilateralImageCreationFunctor
{
  float64 pDomainSigma = 4.0;
  float64 pRangeSigma = 50.0;
  unsigned int pNumberOfRangeGaussianSamples = 100u;
  ...
  auto operator()() const
  {
    ...
    filter->SetDomainSigma(pDomainSigma);
    filter->SetRangeSigma(pRangeSigma);
    filter->SetNumberOfRangeGaussianSamples(pNumberOfRangeGaussianSamples);
    return filter;
  }
};
} // namespace
...
IFilter::PreflightResult ITKBilateralImage::preflightImpl(const DataStructure& dataStructure, const Arguments& filterArgs, const MessageHandler& messageHandler) const
{
  ...
  auto pDomainSigma = filterArgs.value<float64>(k_DomainSigma_Key);
  auto pRangeSigma = filterArgs.value<float64>(k_RangeSigma_Key);
  auto pNumberOfRangeGaussianSamples = filterArgs.value<unsigned int>(k_NumberOfRangeGaussianSamples_Key);
  ...
  // store those actions.
  complex::Result<OutputActions> resultOutputActions = BilateralImage::ITK::DataCheck(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath);
...
Result<> ITKBilateralImage::executeImpl(DataStructure& dataStructure, const Arguments& filterArgs, const PipelineFilter* pipelineNode, const MessageHandler& messageHandler) const
{
   ...
   ***************************************************************************/
  ::ITKBilateralImageCreationFunctor itkFunctor = {pDomainSigma, pRangeSigma, pNumberOfRangeGaussianSamples};
   ...
   ***************************************************************************/
  return BilateralImage::ITK::Execute(dataStructure, pSelectedInputArray, pImageGeomPath, pOutputArrayPath, itkFunctor);
}
} // namespace complex
