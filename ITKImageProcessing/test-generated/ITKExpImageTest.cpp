...
TEST_CASE("ITKExpImageFilter(defaults)", "[ITKImageProcessing][ITKExpImage][defaults]")
{
  ...
  args.insertOrAssign(ITKExpImage::k_OutputImageDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  fs::path baselineFilePath = fs::path(complex::unit_test::k_DataDir.view()) / "JSONFilters/Baseline/BasicFilters_ExpImageFilter_defaults.nrrd";
