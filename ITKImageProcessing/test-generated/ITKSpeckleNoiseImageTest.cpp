...
TEST_CASE("ITKSpeckleNoiseImageFilter(2d)", "[ITKImageProcessing][ITKSpeckleNoiseImage][2d]")
{
  ...
  Result<> compareResult = ITKTestBase::CompareImages(dataGraph, baselineGeometryPath, baselineDataPath, inputGeometryPath, outputDataPath, 100.0);
...
TEST_CASE("ITKSpeckleNoiseImageFilter(3d)", "[ITKImageProcessing][ITKSpeckleNoiseImage][3d]")
{
  ...
  DataPath baselineDataPath = baseLineCellDataPath.createChildPath(ITKTestBase::k_BaselineDataPath);
  Result<> readBaselineResult = ITKTestBase::ReadImage(dataGraph, baselineFilePath, baselineGeometryPath, baseLineCellDataPath, baselineDataPath);
  Result<> compareResult = ITKTestBase::CompareImages(dataGraph, baselineGeometryPath, baselineDataPath, inputGeometryPath, outputDataPath, 200000.0);
  COMPLEX_RESULT_REQUIRE_VALID(compareResult);
}
