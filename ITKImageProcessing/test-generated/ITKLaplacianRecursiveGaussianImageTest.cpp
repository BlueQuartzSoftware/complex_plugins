...
TEST_CASE("ITKLaplacianRecursiveGaussianImageFilter(default)", "[ITKImageProcessing][ITKLaplacianRecursiveGaussianImage][default]")
{
  ...
  args.insertOrAssign(ITKLaplacianRecursiveGaussianImage::k_OutputImageDataPath_Key, std::make_any<DataPath>(outputDataPath));
  ...
  fs::path baselineFilePath = fs::path(complex::unit_test::k_DataDir.view()) / "JSONFilters/Baseline/BasicFilters_LaplacianRecursiveGaussianImageFilter_default.nrrd";
