...
TEST_CASE("ITKSaltAndPepperNoiseImageFilter(2d)", "[ITKImageProcessing][ITKSaltAndPepperNoiseImage][2d]")
{
  ...
  Result<> compareResult = ITKTestBase::CompareImages(dataGraph, baselineGeometryPath, baselineDataPath, inputGeometryPath, outputDataPath, 20);
...
TEST_CASE("ITKSaltAndPepperNoiseImageFilter(3d)", "[ITKImageProcessing][ITKSaltAndPepperNoiseImage][3d]")
{
  ...
  DataPath baselineDataPath = baseLineCellDataPath.createChildPath(ITKTestBase::k_BaselineDataPath);
  Result<> readBaselineResult = ITKTestBase::ReadImage(dataGraph, baselineFilePath, baselineGeometryPath, baseLineCellDataPath, baselineDataPath);
  Result<> compareResult = ITKTestBase::CompareImages(dataGraph, baselineGeometryPath, baselineDataPath, inputGeometryPath, outputDataPath, 10000.0);
  COMPLEX_RESULT_REQUIRE_VALID(compareResult);
}
